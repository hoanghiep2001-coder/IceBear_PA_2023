...
export default class GamePlay extends cc.Component {
  ...
  @property(cc.Node)
  TrainPoint2: cc.Node = null;
  @property(cc.Node)
  TrainPoint3: cc.Node = null;
  ...
  worldPosA: cc.Vec2 = null;
  worldPosC: cc.Vec2 = null;
  worldPosBreakPoint: cc.Vec2 = null;
  ...
  private onHideMaskTouchMove(e: cc.Touch): void {
    this.handPoint.angle = this.handleRotateHand();

    this.worldPosA = this.Point_A.convertToWorldSpaceAR(cc.Vec2.ZERO)
    this.worldPosC = this.Point_C.convertToWorldSpaceAR(this.currentPos);
  ...
  private isLineIntersectingNode(pointA: cc.Vec2, PointC: cc.Vec2, Node: cc.Node): boolean {
    let boundingBox = Node.getBoundingBoxToWorld();

    if (cc.Intersection.lineRect(pointA, PointC, boundingBox)) {
      return true;
    }
    return false;
  }
  ...
  private handleSetPosPointC(touchPos: cc.Vec2): void {
    ...
    && touchPos.x < this.FenceFoot.x || touchPos.y < this.FenceFoot.y
  ) {
    console.log("check3");
  ...
  private handleCheckCollideTop(touchPos: cc.Vec2): void {
    if (!this.FlagHorizontal) this.FlagHorizontal = touchPos.y + 30;
    this.Point_C.x = this.currentPos.x - cc.winSize.width / 2;
  ...
  protected update(dt: number): void {
    ...
    this.handPoint.x = this.Point_C.x;
    this.handPoint.y = this.Point_C.y;

    this.TrainPoint.position = this.train.position;
    this.TrainPoint2.x = this.train.x - 50;
    this.TrainPoint3.x = this.train.x + 50;
    ...
    if (this.isCollideBreakPoint) {
      this.worldPosBreakPoint = this.FenceFoot.convertToWorldSpaceAR(this.BreakPoint);
      if (
        this.isLineIntersectingNode(this.worldPosA, this.worldPosBreakPoint, this.TrainPoint)
        // || this.isLineIntersectingNode(this.worldPosBreakPoint, this.worldPosC, this.Point_Girl)
      ) {
        console.log("Check");
        ...
        this.hanldeCheckCollideGirl(this.currentPos);
        return;
      }
    }


    if (this.worldPosA && !this.isEndGame && !this.isCollideBreakPoint) {
      if (
        this.isLineIntersectingNode(this.worldPosA, this.worldPosC, this.TrainPoint)
        || this.isLineIntersectingNode(this.worldPosA, this.worldPosC, this.TrainPoint2)
        || this.isLineIntersectingNode(this.worldPosA, this.worldPosC, this.TrainPoint3)
        || this.isLineIntersectingNode(this.worldPosA, this.worldPosC, this.checkPoint_3)
      ) {
        console.log("Check");
        if (this.isFail) {
          return;
