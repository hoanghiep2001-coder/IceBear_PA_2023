...
export default class GamePlay extends cc.Component {
    ...
    isFightOfForthHero: boolean = false;
    isFightOfFifthHero: boolean = false;
    isFightOfSixthHero: boolean = false;
    isEndGame: boolean = false;
    isPlayBg: boolean = false;
    midLaneChooseDone: boolean = false;
    ...
    protected onLoad(): void {
        ...
        this.hint_Blinks[1].active = false;
        this.hint_Blinks[2].active = false;
        this.stick_HerosInArea.forEach(hero => {
            hero.active = false;
        });

        this.stick_HerosInArea[0].active = true;
    }

    protected start(): void {
        cc.audioEngine.play(this.AudioManager.bg_Fight, true, 1);
    ...
    private handleTouchEnd(e: cc.Touch): void {
        ...
        if (
            ...
            if (this.midLaneChooseDone) {
                console.log("true");

                this.unActiveHintAndHero(true);
            } else {
                console.log("false");
                this.unActiveHintAndHero(false);
            }

            if (this.coutingTimeDrag >= 2) {
                this.handleMoveHeros();
            }
            ...
            this.hint_Blinks[this.arenaIsChoosing].active = false;
    ...
    private handleHeroTouchStart(heroId: number): void {
        ...
        if (this.midLaneChooseDone) {
            this.arenaIsChoosing = 1;
        }

        if (heroId == 0 && !this.midLaneChooseDone) {
            this.hint_Blinks[2].active = true;
            this.hint_Blinks[2].getComponent(cc.Animation).play("Hint_Blink");
            this.arenaIsChoosing = 2;
        } else {
            this.hint_Blinks[this.arenaIsChoosing].active = true;
            this.hint_Blinks[this.arenaIsChoosing].getComponent(cc.Animation).play("Hint_Blink");
        }
    ...
    private handleMoveHeros(): void {
        ...
        cc.tween(this.stick_HerosInArea[0])
            .to(1, { x: this.monster1Pos.x - 50 })
            .start();
        cc.tween(this.stick_HerosInArea[1])
            .to(1, { x: this.monster2Pos.x - 150 })
            .start();
        cc.tween(this.stick_HerosInArea[2])
            .to(1, { x: this.monster3Pos.x - 100 })
            .start();
        cc.tween(this.stick_HerosInArea[3])
            .to(1, { x: this.monster3Pos.x - 50 })
            .start();
        cc.tween(this.stick_HerosInArea[4])
            .to(1, { x: this.monster2Pos.x - 50 })
            .start();
        cc.tween(this.stick_HerosInArea[5])
            .to(1, { x: this.monster2Pos.x - 100 })
            .start();
    }
    ...
    private handleFight(): void {
        ...
        if (
            this.stick_HerosInArea[3].getBoundingBox()
                .intersects(this.stick_Monsters[2].getBoundingBox()) && !this.isFightOfForthHero) {
            let attackSound = cc.audioEngine.play(this.AudioManager.meleeSound, true, 0.5);
            this.isFightOfForthHero = true;
            this.stick_HerosInArea[3].getComponent(sp.Skeleton).setAnimation(0, "atk_melee", true);
            this.stick_Monsters[2].getComponent(sp.Skeleton).setAnimation(0, "attack", true);

            this.scheduleOnce(() => {
                this.countingFight++;
                cc.audioEngine.stop(attackSound);
                this.stick_HerosInArea[3].active = false;
                this.stick_Monsters[2].getComponent(sp.Skeleton).setAnimation(0, "Idlee", true);
            }, 2)
        }

        if (
            this.stick_HerosInArea[4].getBoundingBox()
                .intersects(this.stick_Monsters[1].getBoundingBox()) && !this.isFightOfFifthHero) {

            let attackSound = cc.audioEngine.play(this.AudioManager.meleeSound, true, 0.5);
            this.isFightOfFifthHero = true;
            this.stick_HerosInArea[4].getComponent(sp.Skeleton).setAnimation(0, "atk_melee", true);
            this.stick_Monsters[1].getComponent(sp.Skeleton).setAnimation(0, "attack", true);

            this.scheduleOnce(() => {
                this.countingFight++;
                cc.audioEngine.stop(attackSound);
                this.stick_HerosInArea[4].active = false;
                this.stick_Monsters[1].getComponent(sp.Skeleton).setAnimation(0, "Idlee", true);
            }, 2)
        }

        if (
            this.stick_HerosInArea[5].getBoundingBox()
                .intersects(this.stick_Monsters[1].getBoundingBox()) && !this.isFightOfSixthHero) {

            let attackSound = cc.audioEngine.play(this.AudioManager.swordSound, true, 0.5);
            this.isFightOfSixthHero = true;
            this.stick_HerosInArea[5].getComponent(sp.Skeleton).setAnimation(0, "atk_sword", true);
            this.stick_Monsters[1].getComponent(sp.Skeleton).setAnimation(0, "attack", true);

            this.scheduleOnce(() => {
                this.countingFight++;
                cc.audioEngine.stop(attackSound);
                this.stick_HerosInArea[5].active = false;
                this.stick_Monsters[1].getComponent(sp.Skeleton).setAnimation(0, "Idlee", true);
            }, 2)
        }
    }
    ...
    private handleEndGame(): void {
        if (this.countingFight >= 2 && !this.isEndGame) {
            ...
            this.stick_Heros[0].active = true;
            this.stick_Heros[1].active = true;
            this.stick_Heros[2].active = true;

            this.stick_Hero_Areas[0].active = true;
            this.stick_Hero_Areas[1].active = true;
            this.stick_Hero_Areas[2].active = true;
            ...
            this.stick_Hero_Areas.forEach(hero => {
                hero.off(cc.Node.EventType.TOUCH_START);
                hero.off(cc.Node.EventType.TOUCH_MOVE);
    ...
    //  ---------- fix - DHH
    private unActiveHintAndHero(flag: boolean): void {
        this.coutingTimeDrag++;

        // unActive Hint Blink
        this.hint_Blinks[this.arenaIsChoosing].active = false;
        this.hint_Blinks[this.arenaIsChoosing].getComponent(cc.Animation).stop();
        ...
        if (flag) {
            ...
            switch (this.heroIsChoosing) {
                case 0:
                    this.stick_HerosInArea[4].active = true;
                    break;
                case 1:
                    this.stick_HerosInArea[1].active = true;
                    break;
                case 2:
                    this.stick_HerosInArea[5].active = true;
                    break;
                default:
                    break;
            }
            ...
            if (this.heroIsChoosing == 0) {
                this.stick_HerosInArea[3].active = true;
            }

            if (this.heroIsChoosing !== 1) {
                this.midLaneChooseDone = true;
            }
        }
